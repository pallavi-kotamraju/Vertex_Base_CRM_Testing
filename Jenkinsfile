...
withCredentials([file(credentialsId: JWT_KEY_CRED_ID, variable: 'jwt_key_file')]) {
	...
	println jwt_key_file
		...
		stage('Create Test Scratch Org') {
			...
			rrsu = bat returnStatus: true, script: "\"${toolbelt}\" force:apex:test:run -n BatchDeletionServiceTest,CreationServiceTest -u ${HUB_ORG}  --codecoverage -w 60 -r human -d test-reports --verbose --wait 10"
